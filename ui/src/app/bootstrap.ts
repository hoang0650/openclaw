...
import { pathForRoute, sessionRouteNamespaceFromPath } from "../app-route-paths.ts";
import {
...
export function bootstrapApplication(
  ...
  const initialSettings = documentMode
    ...
    : persistedSettings;
  const startupSettings = resolveApplicationStartupSettings(initialSettings, startupLocation);
  const earlyBasePath = resolveControlUiBasePath(
    startupSettings.location.pathname || globalThis.location?.pathname || "/",
  );
  const startup = {
    ...startupSettings,
    location: documentMode
      ? startupSettings.location
      : normalizeInitialApplicationLocation(
          startupSettings.location,
          earlyBasePath,
          startupSettings.settings.sessionKey,
        ),
  };
  if (
